...
public class UserActivity {
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		//These conditional statements used above and below are
		//being implemented to get current value of the class
		//instance currently being used while the application is running
		else if(user.getUserRole() == UserRole.MODERATOR){
			if((user.getUsername() == question.getUser().getUsername())||(user.getUserRole() == UserRole.USER)){
				questionService.deleteQuestion(question);
			}
			else{
				System.out.println("Your are not authorised to delete this question");
			}
		}
		else if(user.getUsername() == question.getUser().getUsername()){
			questionService.deleteQuestion(question);
		}
		else{
			System.out.println("Your are not authorised to delete this question");
